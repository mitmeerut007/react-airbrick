  ...
  const fetchImagesAndTags = async () => {
    ...
    try {
      const [imagesResponse, tagsResponse] = await Promise.all([
        axios.get(`http://localhost:5000/api/image/filter?tags=${selectedTags}&page=${currentPage}`, axiosConfig), // Join selectedTags to create a single string
        axios.get("http://localhost:5000/api/image/tags", axiosConfig),
      ]);
